    ...
    try:
        with sync_playwright() as p:
            ...
            if not good_comments and not bad_comments:
                print("⚠️ 未找到任何评论，可能是页面结构变化或该商品无评论")
                return None
                
            return {
                "good_comments": good_comments,
                "bad_comments": bad_comments,
                "params": "从京东页面抓取的商品参数（略）"
            }
    except Exception as e:
...
def crawl_taobao_comments(url: str) -> Optional[Dict[str, Any]]:
    ...
    if not item_id_match:
        # 也可能是其他格式，如 https://item.taobao.com/item.htm?id=123
        item_id_match = re.search(r"item\.htm\?id=(\d+)", url)
    if not item_id_match:
        print("⚠️ 无法从淘宝链接中提取商品ID")
        return None
    
    item_id = item_id_match.group(1)
    # 淘宝评论接口（需要加上referer等头）
    api_url = f"https://rate.tmall.com/list_detail_rate.htm?itemId={item_id}&sellerId=&currentPage=1&pageSize=5"
    headers = {
    ...
    try:
        ...
        for rate in rates[:5]:
            ...
            if rate.get("rateType") == 1:   # 1好评，0中评，-1差评
                good_comments.append(content)
            elif rate.get("rateType") == -1:
                bad_comments.append(content)
        if not good_comments and not bad_comments:
            print("⚠️ 该淘宝商品暂无评论")
            return None
        return {
...
def enhance_table_with_link(base_table: str, extra_data: Optional[Dict[str, Any]], source_type: str) -> str:
    """用爬取的真实数据完善表格，如果extra_data为None则返回原表"""
...
"""
    ...
    if result:
        return result
    # 降级：直接在原表末尾追加行（仅当有真实数据时）
    if source_type in ("taobao", "jd"):
        good = extra_data.get('good_comments', [''])[0][:30] if extra_data.get('good_comments') else "无"
        bad = extra_data.get('bad_comments', [''])[0][:30] if extra_data.get('bad_comments') else "无"
        return base_table + f"\n| 用户真实反馈 | {good} | {bad} | 来自真实用户评价 |"
    else:
        title = extra_data.get('title', '')[:20]
        comments = ', '.join(extra_data.get('comments', []))[:50]
        return base_table + f"\n| 社媒舆论 | {title} | 点赞{extra_data.get('likes', 0)} | {comments} |"
